  ...
  return (
    <Card className={`w-56 ${className}`}>
      <CardContent className="p-3">
        <div className="text-muted-foreground mb-2 text-xs font-medium">Flow Overview</div>
        <div
          className="bg-muted/30 relative rounded border"
          style={{
        ...
        >
          ...
          {flow.nodes.map(node => (
            <div
              key={node.id}
              className={`absolute h-3 w-3 cursor-pointer rounded-sm transition-all ${getNodeTypeColor(
                node.type
              )} ${
                selectedNodeId === node.id ? 'ring-primary scale-125 ring-2' : 'hover:scale-110'
              }`}
              style={{
