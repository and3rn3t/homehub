...
interface SystemAlert {
  id: string
  type: 'device-offline' | 'low-battery' | 'weak-signal' | 'system-error' | 'maintenance' | 'security'
  title: string
  message: string
...
export function NotificationCenter() {
    ...
    if (expiredAlerts.length > 0) {
      setAlerts(currentAlerts => 
        currentAlerts.filter(alert => 
          !(alert.autoExpire && alert.expiresAt && alert.expiresAt < now)
        )
      )
    }
  }, [alerts, setAlerts])
    ...
    const interval = setInterval(() => {
      ...
      if (Math.random() < 0.05) { // 5% chance every 10 seconds
        const alertTypes: SystemAlert['type'][] = [
          'device-offline', 'low-battery', 'weak-signal', 'system-error', 'maintenance'
        ]
  ...
  const getAlertTitle = (type: SystemAlert['type']) => {
    switch (type) {
      case 'device-offline': return 'Device Offline'
      case 'low-battery': return 'Low Battery Warning'
      case 'weak-signal': return 'Weak Signal Detected'
  ...
  const getAlertMessage = (type: SystemAlert['type']) => {
    switch (type) {
      case 'device-offline': return 'A smart home device has gone offline and may need attention'
      case 'low-battery': return 'One or more devices have critically low battery levels'
      case 'weak-signal': return 'Network connectivity issues detected on some devices'
      case 'system-error': return 'An unexpected system error has occurred'
      case 'maintenance': return 'Scheduled maintenance will begin shortly'
      case 'security': return 'Unusual activity detected on your security system'
      default: return 'A system notification requires your attention'
    }
  }
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed top-4 right-4 w-96 max-w-[calc(100vw-2rem)] z-50"
        >
                              ...
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <Badge 
                                  ...
                                  className={`h-4 text-xs ${
                                    alert.acknowledged ? 'bg-muted text-muted-foreground' : severityBadgeColors[alert.severity]
                                  }`}
                                >
                                  {alert.severity}
                                </Badge>
